...
const EQUILIBRIUM_CO2_PARAMETER = 5.494847205475377e-8;
...
class Terraforming extends EffectableEntity{
  ...
  calculateInitialValues() {
    ...
    update(deltaTime) {
      ...
      this.simulateSurfaceWaterFlow(deltaTime); // Call for Step 4
    ...
    // Simulates the flow of surface liquid water between adjacent zones
    simulateSurfaceWaterFlow(deltaTime) {
        const flowRateCoefficient = 0.005; // Adjust this to control flow speed (fraction per second)
        ...
        zones.forEach(zone => {
            ...
            if (outflowTarget) {
                const currentAmount = this.zonalWater[zone].liquid || 0;
                const neighborAmount = this.zonalWater[outflowTarget].liquid || 0;

                // Simple model: flow occurs if current zone has more water than neighbor
                // More complex models could use elevation, coverage percentage etc.
        ...
        zones.forEach(zone => {
            if (flowChanges[zone].liquid !== 0) {
                this.zonalWater[zone].liquid += flowChanges[zone].liquid;
                this.zonalWater[zone].liquid = Math.max(0, this.zonalWater[zone].liquid); // Ensure non-negative
            }
        });
    }

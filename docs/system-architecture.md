...
- Surface optics use `climate.surfaceModel` and `climate.surfaceAlbedo` in `terraforming-parameters.js`. `surface-albedo.js` resolves resource-keyed coverages for both runtime and random-world generation. Liquid hydrogen claims area first, other exposed volatiles share the remainder, biomass claims up to 75% of the remaining area, and deposits share the substrate below those coverings. Visible fractions plus uncovered ground total one. Land, hazardous biomass, hazardous machinery, waste resources (scrap metal, garbage, trash, junk and radioactive waste) and buried reservoirs have no optical contribution.
- Resource coverage scales belong in `planet-resource-parameters.js`: zonal resources use `zonalConfig.coverageScale` (and optional per-key `coverageScales`), while global deposits use top-level `coverageScale`. Runtime and world generation read these definitions. Global deposits retain their existing resource/save representation; derive their uniform optical footprint from tons per geometric square metre using the configured coverage scale. Do not use expanded or available construction land. Planet `celestialParameters.surfaceAlbedo` entries override material coefficients; biomass uses the active life design. Loose rocks use the world's unmodified rock albedo by default, while uncovered ground uses zonal Dust Factory albedo.
- Optical masking does not remove thermal inventories: liquid heat capacity continues to use raw coverage and liquid mass. Ground deposits do not displace optical liquid/ice shares or alter thermal inventory. Live climate, mirror projections and the surface breakdown must use the same zonal ground and material albedos.

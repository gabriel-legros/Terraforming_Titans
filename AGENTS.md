...
- Nanoworlds are a Steam-exclusive `5T` advanced research specialization unlocked after Nanotechnology Stage IV. Starting one requires a terraformed world with nanobots at the current cap and no competing specialization. Completion consumes all biomass, forces biomass maximum density to zero, multiplies the current world's nanobot density cap by `100`, removes and disables all ground colonies while preserving Aerostat Colonies, and awards `max(1, log10(nanobots) / 4)` Nanoworld Points on travel plus `10%` per hazard. Its shop has `+1%` density and Stage I-IV full-effect upgrades, each capped at `+900%`. Completed Nanoworlds replace all terrain and water visuals with a dark iridescent, emissive nanite-ocean texture beneath clouds and atmospheric effects.

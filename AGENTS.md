...
- Dyson Sphere now tracks a derived sphere count (`0` before frame completion, then `ceil(power / 2e25)`), enforces a default max of 1 sphere (max power `2e25`), unlocks expansion to 100 billion spheres via advanced research `Additional Dyson Spheres`, and adds a fixed superalloy surcharge to Dyson Sphere collector costs once expanding past 1-sphere power.

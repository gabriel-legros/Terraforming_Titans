...
- SMBH shellworlds unlock the Birch World mega project. It tracks up to 1500 shell layers, starts with the original layer, uses `ShellworldGravityHelper` for each next-layer cost, sets current land to summed shell-layer hectares, and updates artificial world value as total land divided by 50B. Birch World is preserved with the SMBH snapshot and must not be compressed into artificial history summaries.

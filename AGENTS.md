...
- Build scripts live under `scripts/`: `build-browser.sh` copies the browser build to `dist/browser`, `build-steam.sh` generates the production Steam build for AppID `4864000` at `dist/steam-win-unpacked`, and `build-steam-playtest.sh` generates the Steam Playtest build for AppID `4876760` at `dist/steam-playtest-win-unpacked`.
...
### Difficulty Settings
- Difficulty settings include persisted multipliers for building costs, global research output, worker requirements, project duration, population growth, life growth, maintenance costs, spaceship energy costs before/after Space Elevator, advanced research gain, Galaxy fleet capacity, Galaxy threat scaling, artificial world construction time, and the Post-World 4 RWG Rewards cap. Defaults are 1 for multipliers and null/blank for uncapped RWG rewards. Building/project duration clamps to at least 0.01, artificial world construction time clamps to at least 0.0001, research/pop/life/maintenance/spaceship/advanced research/Galaxy multipliers clamp to at least 0, and RWG Rewards cap clamps to whole numbers at least 0. Effects are applied through the shared effect system when settings change and during the normal manager effect reapply pass on load/travel.

...
- Local mod development: `electron/mods/mod-service.cjs` discovers direct child folders under `TERRAFORMING_TITANS_MODS_DIR`, Electron user data `mods/local`, and unpackaged-repo `local-mods`. Each mod uses `terraforming-titans.mod.json`; the current local slice supports declarative JSON patches and renderer-file replacements, with later `loadOrder`/id entries winning. Steam Workshop discovery/publishing, dependencies, additive scripts/styles, save isolation, and the Mods UI are not implemented yet. See `docs/modding/local-development.md`.
...
- Any active local mod disables Steam achievement publication for that Electron session. The active read-only diagnostic summary is `activeModSession`; log only compact mod ids/error counts rather than full session or patch graphs.

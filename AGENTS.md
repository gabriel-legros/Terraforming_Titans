...
- Script Automation adds a manually enabled top Automation-card script runner with a localized attached header tooltip, dropdown-built conditions, expandable/condensed IF/WAIT/ACTIONS lines, IF-only Else Actions, GOTO, Sleep actions, per-line enable toggles/descriptions, paused single-step execution, highlighted current-line display, live selected-value readouts, and one-shot application of building/project/colony/research presets and combinations; scripts persist through save/load, can be queued to run after next-travel preset application, and execute with per-tick GOTO/action limits. Script references include surface-resource coverage, terraforming status/specialization, and shortened celestial-parameter selectors.

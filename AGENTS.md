...
- The Factory heating difficulty setting converts local building and colony `colony.energy` consumption into surface heat using each structure's `factoryHeatCoefficient`. Most local consumers use `1`, chemical-storage or directed-energy processes use lower values, Superalloy Foundries use `0.01`, and Hyperion Lanterns use `0` to avoid double-counting direct heating. Mega Heat Sinks offset core heat first, then factory heat, and only surplus cooling changes temperature.

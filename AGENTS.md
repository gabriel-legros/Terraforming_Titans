...
- Advanced oversight limits tropical/temperate/polar targets to priorities 1-3. `Allow available to heat` is fixed at priority 4 and reserves only the mirrors/lanterns estimated to contribute toward the next tick's temperature trend, allowing a priority-5 melt target to use the remaining facility capacity; melt targets retain priorities 1-5.

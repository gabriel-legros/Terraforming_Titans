...
- Advanced research includes `Teleporters` (100Q), applying a Space Storage flag that unlocks a transfer method selector. Teleporter transfers use a fixed rate or per-worker rate, ignore Kessler, and pay `100x` the normal shipment energy before flat spaceship energy penalties.

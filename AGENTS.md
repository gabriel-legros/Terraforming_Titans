...
- `liquidHydrogen` now has a supercritical hydrogen repartition cycle: below the real hydrogen critical point it uses hydrogen saturation-pressure targets, while above the critical point it relaxes atmospheric hydrogen toward a pressure boundary anchored to the critical point and the Jovian `10 GPa / 3000 K` supercritical transition, moving mass between atmospheric hydrogen and the compressed `liquidHydrogen` reservoir instead of using a fake high-temperature boiling curve. It also participates in zonal hydrology-style lateral flow, so the hydrogen envelope can redistribute across zones even when the global atmospheric pressure is already in balance, and its zonal surface ledger now persists through save/load.

...
- SMBH shellworlds save a lightweight per-world snapshot on departure and restore it on return. The snapshot keeps resource values, terraforming state including temperatures and zonal surface state, building/colony state, colony sliders, nanocolony settings, orbital allocation, resources/infrastructure project state plus Mega Heat Sink and Space Storage, and completed research; other mega/giga/tera project state is excluded. Restore must reapply manager/research/WGC/project effects to the recovered objects.

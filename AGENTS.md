...
- Lifters now use per-recipe assignment controls (including Strip Atmosphere/Hydrogen/Methane/Ammonia), with per-recipe complexity, auto-assignment weights, and per-recipe productivity/throughput handling so mixed allocations run simultaneously.
...
- Advanced research now includes `Manufacturing Worlds` (500M), unlocking a zero-cost world-specialization project (mutually exclusive with Bioworld/Foundry/Holy World): completed departures grant MP as `max(1, log10(population))`, bank departed population permanently as cumulative manufacturing potential, and enable global assignment-based space-storage manufacturing for Glass/Components/Electronics/Superconductors/Superalloys with MP-shop scaling (+1% production and consumption per level, max 900 each).

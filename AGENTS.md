...
- SpaceManager records departed-world `populationCapacity` beside departed colonists as `max(colonist cap, 10 * land value)`, aggregates compacted random/artificial capacity totals, maintains `galacticPopulation` / `galacticPopulationCapacity` attributes, updates those attributes incrementally on travel unless missing/zero, and exposes total departed population/capacity getters; Followers derives galactic population from SpaceManager while retaining its own believer accounting.

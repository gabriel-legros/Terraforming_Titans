...
- AI language example mods live under `examples/local-mods/ai-*-translation`. Their `patches/current-language.json` and `patches/story-language.json` files mirror `src/js/lang/current-language.js` and `src/js/lang/story-language.js` respectively so source and translation keys stay directly diffable. Run `bash scripts/generate-base-english-language.sh` to generate the matching canonical English files under `artifacts/translation-work/base-english/`; run `npm run mod:seed-english-translations` to regenerate the placeholder mods (retaining only each locale's `meta.code`), then run `npm run mod:validate-translations`.

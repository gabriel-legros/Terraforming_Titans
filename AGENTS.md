...
- Diskworld Terraforming is a disk-only project that pumps space-storage hydrogen into the disk until fill gravity reaches 1g; pumping energy scales with internal hydrogen pressure, and disk gravity updates from fill progress.
- Shellworld and diskworld spaceship energy effects are per-ton penalties rather than base energy multipliers. Shellworlds add `max(radius - 1, 0) * 100,000` energy per ton; diskworlds add `fillRate * max(radius - 1, 0) * 100,000` energy per ton, where fill rate is current structural fill progress toward 1g. These flagged effects do not apply to import projects while their target is Space Storage.

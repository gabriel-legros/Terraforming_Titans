...
- Dynamic-mass worlds now expose `Planetary Mass` as an underground resource. It stays hidden unless `specialAttributes.dynamicMass` is enabled on that world, its value is synced as current total planetary mass minus current tracked surface-resource mass, so the initial value is generated world mass minus initial surface resources and direct import/disposal flows show visible rate changes on the non-surface bulk mass pool.

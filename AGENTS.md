...
- Galaxy Operations now has a persisted `Global Auto Operations` selector (`All sectors`, `Off`, `Force all off`) plus a shared auto-launch multiplier input. `All sectors` hard-overrides per-sector auto toggles and launches eligible frontier operations by cheapest required fleet power first; `Off` leaves individual sector auto settings active; `Force all off` suppresses individual auto settings without clearing their saved checkbox state.

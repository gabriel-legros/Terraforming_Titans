...
- Atlas challenge world `TheRealPoseidon` disables aerostat powered flight only on that world, and its Challenge Rules text states: `A certain World 14 upgrade on aerostats is disabled because HOPE did not have access to it at the time.`
  ...
  - Loading a save now captures previously saved auto-travel enabled, scripting enabled, and scripting running states, force-disables those automations for soft-lock safety during load, then shows a post-load Yes/No restore popup. Choosing Yes restores the saved states exactly (including scripting enabled-but-not-running); choosing No keeps them off.

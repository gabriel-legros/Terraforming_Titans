...
- Mod loading: `electron/mods/mod-service.cjs` discovers direct child folders under `TERRAFORMING_TITANS_MODS_DIR`, Electron user data `mods/local`, and unpackaged-repo `local-mods`. `electron/mods/workshop-service.cjs` enumerates Steam subscriptions at Steam-build startup, requests missing/outdated items, and resolves their installed folders through Steamworks. Each mod uses `terraforming-titans.mod.json`; local and Workshop mods share declarative JSON patches, renderer-file replacements, and deterministic `loadOrder`/id ordering. In-game publishing, dependencies, additive scripts/styles, save isolation, and the Mods UI are not implemented yet. See `docs/modding/local-development.md`.

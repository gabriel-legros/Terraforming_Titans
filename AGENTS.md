...
- Olympus defines world-level disabled features in `specialAttributes.disabledFeatures`: Research, HOPE, and Colony tabs are hidden; advanced research effects are suppressed; Space keeps only Story; Terraforming keeps only World; Nanocolony/Followers managers are disabled; funding, research, advanced research, alien artifacts, antimatter, and Space Energy resources are hidden; mega/giga/tera special-project tabs and projects are disabled; Cargo Rockets and Import Colonists are permanently disabled. World disables default to priority `2`; normal enable effects default to priority `1`, while later story unlocks can use an enable effect with `priority`/`enablePriority`/`level` above `2` to override the world disable. A high-priority `nanotechManager` enable also restores the Colony tab and Nanocolony subtab.

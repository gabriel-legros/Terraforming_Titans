...
- The generated Lorem ipsum example language lives under `examples/local-mods/lorem-ipsum-language`. Regenerate its complete patch with `npm run mod:generate-lorem` after `src/js/lang/current-language.js` changes; `scripts/generate-lorem-language-mod.js` verifies that every string changes and that all interpolation tokens and collection shapes remain intact.

...
var SpaceMirrorAdvancedOversightModule = (typeof globalThis !== 'undefined' && globalThis.SpaceMirrorAdvancedOversight)
  ...
  : null;
try {
  if (!SpaceMirrorAdvancedOversightModule && typeof require === 'function') {
    ({ SpaceMirrorAdvancedOversight: SpaceMirrorAdvancedOversightModule } = require('./SpaceMirrorAdvancedOversight.js'));
  }
} catch (e) {}
  ...
  if (typeof projectManager !== 'undefined' &&
    ...
    if (focusPower > 0) {
      const zonesData = ['tropical','temperate','polar'].map(z => ({
        ...
        ice: terraforming.zonalSurface.ice[z]
      })).filter(z => z.ice > 0);
      ...
      for (const z of zonesData) {
        ...
        terraforming.zonalSurface.ice.change(z.zone, -meltHere);
        terraforming.zonalSurface.liquidWater.change(z.zone, meltHere);
        focusMeltAmount += meltHere;
      }

      if (focusMeltAmount > 0) {
        if (resources.surface?.ice) resources.surface.ice.value -= focusMeltAmount;
        if (resources.surface?.liquidWater) resources.surface.liquidWater.value += focusMeltAmount;
      }
    }
  }

...
class Building extends EffectableEntity {
  ...
  getSingleProductionResource() {
    if (this.category === 'colony') {
      return null;
    }
    ...
    for (const category in production) {
      const categoryMap = production[category];
      if (!categoryMap) continue;

      for (const resource in categoryMap) {
        const amount = categoryMap[resource];
    ...
    if (foundResource === 'energy') {
      return null;
    }

    return { category: foundCategory, resource: foundResource };
  }

  getMaintainAutoBuildTarget(desiredRate) {
    const output = this.getSingleProductionResource();
    if (!output || !resources) {
      return this.count;
    }

    const resourceObj = resources[output.category]?.[output.resource];
    if (!resourceObj) {
      return this.count;
    }

    const targetRate = Number.isFinite(desiredRate) && desiredRate > 0 ? desiredRate : 0;
    if (targetRate <= 0) {
      return this.count;
    }

    const netRate = resourceObj.projectedProductionRate - resourceObj.projectedConsumptionRate;
    if (netRate >= targetRate) {
      return this.count;
    }

    const sourceRate = resourceObj.projectedProductionRateBySource?.[this.displayName];
    const activeCount = Math.max(this.active, this.count, 1);
    let perBuildingRate = Number.isFinite(sourceRate) && sourceRate > 0
      ? sourceRate / activeCount
      : 0;

    if (!Number.isFinite(perBuildingRate) || perBuildingRate <= 0) {
      const base = this.production?.[output.category]?.[output.resource] || 0;
      if (base > 0) {
        const ratio = this.getProductionRatio ? this.getProductionRatio() : 1;
        const productionMultiplier = this.getEffectiveProductionMultiplier
          ? this.getEffectiveProductionMultiplier()
          : 1;
        const resourceMultiplier = this.getEffectiveResourceProductionMultiplier
          ? this.getEffectiveResourceProductionMultiplier(output.category, output.resource)
          : 1;
        const automationMultiplier = this.getAutomationActivityMultiplier
          ? this.getAutomationActivityMultiplier()
          : 1;
        perBuildingRate = base * ratio * productionMultiplier * resourceMultiplier * automationMultiplier;
      }
    }
    ...
    const missingRate = targetRate - netRate;
    const additionalNeeded = Math.ceil(missingRate / perBuildingRate);
    return this.count + Math.max(additionalNeeded, 0);
  }

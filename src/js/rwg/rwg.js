  ...
  atmosphere: {
    templates: {
      ...
      "ammonia-rich": {
        ...
        mix: {
          inertGas: 0.65,
          atmosphericAmmonia: 0.25,
          atmosphericMethane: 0.05,
          carbonDioxide: 0.03,
          atmosphericWater: 0.01,
          hydrogen: 0.01
        }
      }
    },
...
const RWG_DOMINION_ORDER = ['human', 'gabbagian', 'ammonia', 'oommaa', 'klishy', 'kerati', 'shrilek', 'vanadophore', 'yggies', 'shiivert'];
...
function orderHazardList(hazards) {
  const ordered = [];
  const seen = new Set();
  RWG_HAZARD_ORDER.forEach((hazard) => {
    if (hazards.includes(hazard) && !seen.has(hazard)) {
      seen.add(hazard);
      ordered.push(hazard);
    }
  });
  hazards.forEach((hazard) => {
    if (hazard && !seen.has(hazard)) {
      seen.add(hazard);
      ordered.push(hazard);
    }
  });
...
function formatHazardList(hazards) {
  const list = orderHazardList(normalizeHazardList(hazards));
...
function parseSeedSpec(seedInput) {
  let base = "";
  let ann = {}; // { target, type, orbitPreset, hazards, aAU }
  let seedInt;
  if (typeof seedInput === "string") {
    ...
    for (let i = 1; i < segs.length; i++) {
      ...
      if (seg.includes("=")) {
        ...
        else if (k === "hazard" || k === "haz" || k === "feature") ann.hazards = normalizeHazardList(v);
        else if (k === "d" || k === "au") {
    ...
    if (unkeyed.length) {
      ...
      if (p3) ann.hazards = normalizeHazardList(p3);
    }
...
function applyHazardPresets(hazardKeys, { landHa, params, surface, zonalSurface }) {
  ...
  const hazards = {};
  list.forEach((hazardKey) => {
    if (!hazardKey || hazardKey === 'none') return;
...
function buildPlanetOverride({ seed, star, aAU, isMoon, forcedType, forcedHazards, roguePulsarStar }, params) {
  ...
  const ammoniaFraction = Number.isFinite(surfaceFractions.ammonia) ? surfaceFractions.ammonia : 0;
  const liquidFraction = Math.min(1, Math.max(0, oceanFraction + hydroFraction + ammoniaFraction));
  if (hazardOverride) {
 ...
 // Randomized, seed-stable ore + geothermal
  ...
  const underground = {
    ore: {
      ...
      hasCap: true,
      areaTotal: moltenOreDisabled ? 0 : areaTotal,
      unlocked: false
    },
    ...({
      geothermal: {
  ...
  const overrides = {
    ...
    celestialParameters: {
      ...
      mass: bulk.mass,
      baseGravity: bulk.gravity,
      baseRadius: bulk.radius_km,
      baseMass: bulk.mass,
      baseLand: landHa,
      ...
      sector: sectorLabel,
      rogue: isStarlessRogueWorld,
      roguePulsar: isRoguePulsarWorld,
      coreHeatFlux
    },
    star: starOverride,
...
class RwgManager extends EffectableEntity {
  ...
  generateRandomPlanet(seed, opts = {}) {
    ...
    const seedHazards = seedAnn?.hazards || seedAnn?.hazard;
    const managerHazards = this.enabledHazards && this.enabledHazards.length ? this.enabledHazards : null;
    let forcedHazards = normalizeHazardList(opts.hazards ?? opts.hazard ?? seedHazards ?? managerHazards);
    forcedHazards = orderHazardList(forcedHazards);
    if (!this.isFeatureUnlocked('hazards')) forcedHazards = [];
    forcedHazards = forcedHazards.filter((hazard) => this.isHazardUnlocked(hazard));
    ...
    return {
      ...
      hazard: forcedHazards.length ? forcedHazards.slice() : 'none',
      hazards: forcedHazards.slice(),
      seedInt: S,
      seedString: canonicalSeed,
      override,

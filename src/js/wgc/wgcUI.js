...
function isWGCLogPinned(container) {
  if (!container) return false;
  const remaining = container.scrollHeight - container.scrollTop - container.clientHeight;
  return remaining <= 4;
}

function scrollWGCLogToBottom(target) {
  if (!target) return;
...
const facilityDescriptions = {
  ...
  shootingRange: 'Boosts Power for challenges by 1% per level.\nProvides 1 / 2 / 3 rerolls at 10 / 25 / 50 for Power checks and Combat challenges (one per challenge). Level 100 converts a rerolled failure into a success.',
  obstacleCourse: 'Boosts Athletics for challenges by 1% per level.\nGrants 1 / 2 / 3 rerolls at 10 / 25 / 50 for individual or team Athletics checks (one per challenge). Level 100 converts a rerolled failure into a success.',
  library: 'Boosts Wit for challenges by 1% per level.\nSupplies 1 / 2 / 3 rerolls at 10 / 25 / 50 for team Wit and Science challenges (one per challenge). Level 100 converts a rerolled failure into a success.'
};
...
function invalidateWGCTeamCache() {
  const previous = teamElements.slice();
    ...
    if (logContainer) {
      ...
      if (!logContainer._wgcScrollHandler) {
        logContainer.addEventListener('scroll', () => {
          const idx = Number(logContainer.dataset.teamIndex);
          if (Number.isNaN(idx)) return;
          const target = teamElements[idx];
          if (!target) return;
          target.logPinned = isWGCLogPinned(logContainer);
        });
        logContainer._wgcScrollHandler = true;
      }
    }
  });
}
...
function createFacilityItem(key, label) {
  ...
  progress.classList.add('wgc-facility-progress');
  const fill = document.createElement('div');
  fill.classList.add('wgc-facility-progress-fill');
  progress.appendChild(fill);
  [10, 25, 50, 100].forEach(value => {
    const marker = document.createElement('div');
    marker.classList.add('wgc-facility-progress-marker');
    if (value === 100) {
      marker.classList.add('wgc-facility-progress-marker-end');
  ...
  facilityElements[key] = { button, level, fill };
  return div;
}
...
function updateWGCUI() {
  ...
  for (const key in facilityElements) {
    ...
    if (el.fill) {
      el.fill.style.width = `${Math.min(Math.max(lvl, 0), 100)}%`;
    }
  }
    ...
    if (logEl) {
      ...
      if (logVisible) {
        if ((contentChanged && visibleCount > 0 && refs.logPinned !== false && wasPinned) || forceScroll) {
          ...
          refs.logPinned = isWGCLogPinned(logContainer);
        }
      }
      if (forceScroll) {
        wgcPendingLogScroll.delete(tIdx);
      }
    }

...
describe('natural magnetosphere integration', () => {
  test('terraforming treats natural magnetosphere as magnetic shield', () => {
    const terra = new Terraforming({}, {
      radius: 1,
      distanceFromSun: 1,
      gravity: 1,
      hasNaturalMagnetosphere: true,
    });
    expect(terra.isBooleanFlagSet('magneticShield')).toBe(true);
    expect(terra.getMagnetosphereStatus()).toBe(true);
  });

  test('magnetosphere box indicates natural magnetosphere', () => {
    ...
    ctx.terraforming = {
      ...
      orbitalRadiation: 0,
      radiationPenalty: 0,
      celestialParameters: { albedo: 0, gravity: 1, radius: 1, surfaceArea: 1, hasNaturalMagnetosphere: true },
    ...
    const statusEl = dom.window.document.getElementById('magnetosphere-status');
    expect(statusEl.textContent).toMatch(/Natural magnetosphere/);
  });
});

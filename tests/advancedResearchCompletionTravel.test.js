...
describe('Completed advanced research persists across planet travel', () => {
  test('traveling does not reset completed advanced research', () => {
    ...
    vm.runInContext("researchManager.completeResearchInstant('modular_nuclear_reactor');", ctx);
    vm.runInContext('spaceManager.updateCurrentPlanetTerraformedStatus(true);', ctx);

    vm.runInContext("selectPlanet('titan');", ctx);

    const researched = vm.runInContext("researchManager.getResearchById('modular_nuclear_reactor').isResearched", ctx);

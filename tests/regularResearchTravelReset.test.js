...
describe('Regular research resets across planet travel', () => {
  test('traveling resets completed regular research', () => {
    ...
    vm.runInContext("researchManager.completeResearchInstant('solar_efficiency');", ctx);
    vm.runInContext('spaceManager.updateCurrentPlanetTerraformedStatus(true);', ctx);

    vm.runInContext("selectPlanet('titan');", ctx);

    const researched = vm.runInContext("researchManager.getResearchById('solar_efficiency').isResearched", ctx);
